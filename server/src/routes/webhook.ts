...
async function checkIfCommenterFollowsBusiness(
    ...
    try {
        ...
        const { data: discoveryData } = await axios.get(discoveryUrl, { params: discoveryParams });
        ...
        const { data: checkData } = await axios.get(checkUrl, { params: checkParams });
    ...
    try {
      ...
      for (const auto of automations) {
        ...
        try {
          ...
          if (auto.followForDM) {
            const isFollowing = await checkIfCommenterFollowsBusiness(
              username,
              igBusinessId,
              pageAccessToken
            );
